...
기능적 요구사항
	...
	6.	각 CCTV 장치의 상태 및 알림 내역을 대시보드에서 확인할 수 있다.
	7.	담당자가 특정 CCTV 장치의 문제 이력을 조회할 수 있다.

비기능적 요구사항

 트랜잭션
- ICMP 핑 테스트 실패 시 알림 전송이 반드시 이루어져야 한다. (Sync 호출)
 장애격리
...
분석 및 설계
	...
	헥사고날 아키텍처
	•	헥사고날 아키텍처 다이어그램이 프로젝트의 각 도메인을 명확하게 나타내고 있는가?

구현

	[DDD]
	•	네트워크 감지, 알림, 문제 해결의 도메인 간 관계를 헥사고날 아키텍처에 맞추어 구현하였는가?
	•	핑 테스트와 HLS 체크 로직을 비동기적으로 처리하여 성능과 장애 격리 기능을 제공하였는가?
	Request-Response 방식의 서비스 중심 아키텍처 구현
	•	서비스 간 호출을 Service Discovery 및 RESTful API를 통해 구현하였는가?
	•	장애 발생 시 서킷브레이커를 통해 문제를 감지하고 격리할 수 있는가?
	이벤트 드리븐 아키텍처의 구현
	•	알림 서비스가 Kafka 등 이벤트 중심으로 구현되어 담당자에게 메시지를 전달하는가?
	•	이벤트 메시지가 특정 CCTV 장치의 문제에 제대로 연결되도록 Correlation-key를 구현하였는가?

운영

	SLA 준수
	•	서비스의 건강 상태를 지속적으로 모니터링하고, Liveness Probe로 문제를 감지했을 때 자동 복구가 이루어지는가?
	•	알림 시스템이 장애 상황에서도 빠르게 복구될 수 있는가?
	무정지 운영 CI/CD
	•	Readiness Probe를 통해 시스템이 완전하게 준비된 후 새로운 버전으로의 롤링 업데이트가 이루어지는가?
